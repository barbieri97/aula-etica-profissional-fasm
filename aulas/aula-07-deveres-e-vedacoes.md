---
theme: none
title: Aula 07 · Deveres e vedações
info: As 29 alíneas dos Arts. 1º e 2º do Código, lidas na íntegra, com as
  resoluções que as detalham e um estudo de caso para decidir se houve falta ética
date: "2026-09-17"
author: FASM · Ética Profissional
colorSchema: light
download: true
themeConfig:
  # Lido por aulas/global-top.vue — é o texto do rodapé de todo slide.
  rodape: FASM · Ética Profissional
layout: capa
kicker: Aula 07
subtitle: 'A primeira família do capítulo de conduta — doze deveres e dezessete
  vedações. É o <span class="ds-em">verbo</span> que separa os dois artigos, e é
  daqui que sai quase toda representação ao Conselho.'
meta: CEPP · Res. CFP 010/2005, Arts. 1º e 2º
---

<!--
Primeira das sete aulas sobre o capítulo de conduta do Código. A aula 06 leu os
princípios; esta começa a leitura do que se cobra.

Quatro blocos: os doze deveres do Art. 1º (35 min), as dezessete vedações do Art.
2º (30 min), as resoluções que detalham algumas alíneas (10 min) e o estudo de
caso (15 min).

O estudo de caso é o formato fixo da série: apresenta-se a situação e a turma
decide se houve falta ética, dizendo qual artigo e qual alínea a caracterizam. O
veredito e a fundamentação estão nas notas do slide que fecha o bloco.
-->

---
layout: roteiro
kicker: Aula 07
title: O caminho de hoje
itens:
  - { tema: Os doze deveres, desc: "Art. 1º, alíneas “a” a “l”" }
  - { tema: As dezessete vedações, desc: "Art. 2º, alíneas “a” a “q”" }
  - { tema: As resoluções que detalham, desc: orientação sexual, identidade de gênero, documentos e testes }
  - { tema: Estudo de caso, desc: houve falta ética? qual artigo a caracteriza? }
---

<!--
Contrato da aula: hoje se lê o miolo do Código. Ninguém decora alínea; sai-se
sabendo reconhecer a família a que um caso pertence e onde procurar.

Tempos: 35 · 30 · 10 · 15.
-->

---
layout: esquema
imagem: /mapa-arts-1-a-20.svg
title: Onde esta aula fica
legenda: O agrupamento é da aula — o Código não subdivide o capítulo, ele vai do Art. 1º ao Art. 20 em sequência.
fonte: CFP, 2005, Das Responsabilidades do Psicólogo, p. 8–15.
---

Hoje é a **primeira família**: os Arts. 1º e 2º, que são 29 das 60 alíneas do
capítulo inteiro.

<!--
Este mapa abre as sete aulas da série, e a cada encontro se marca em voz alta qual
é a família do dia. Hoje, a primeira caixa.

Diga que o agrupamento é nosso, não do Código: quem abrir o PDF vai encontrar 20
artigos em fila, sem subtítulo nenhum.

Vale antecipar o desenho da série: cada família ganha um encontro, e os Arts. 21 a
25 — infração e penalidade — fecham o ciclo na última.
-->

---
layout: secao
numero: "01"
title: Os doze deveres
note: Art. 1º, alíneas “a” a “l”. O que o psicólogo tem de fazer.
---

---
layout: documento
artigo: Art. 1º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: São deveres fundamentais dos psicólogos
fonte: CFP, 2005, p. 8 — alíneas "a" e "b" de doze
---
**a)** Conhecer, divulgar, cumprir e fazer cumprir este Código;

**b)** Assumir responsabilidades profissionais somente por atividades para as
quais esteja **capacitado pessoal, teórica e tecnicamente**;

<!--
Começa a leitura. Combine o ritmo: cada tela se lê em voz alta e se comenta em
duas frases; o comentário longo fica para o slide do caso.

A alínea "a" tem um verbo a mais do que a turma espera: FAZER CUMPRIR. O dever
não termina na própria conduta — e é daí que sai a alínea "l", oito telas adiante.

A alínea "b" é a da competência, e a que mais aparece em processo ético de
recém-formado: aceitar demanda para a qual não se tem formação. Note as três
palavras — pessoal, teórica E tecnicamente. Estar bem tecnicamente não basta se a
demanda toca algo que o profissional não sustenta pessoalmente.

Ela volta no slide de exemplo deste artigo.
-->

---
layout: default
---
<Grade :cols="2">
<Artigo numero="Art. 1º · c" norma="CEPP 2005 · p. 8">

**c)** Prestar serviços psicológicos de qualidade, em condições de trabalho
dignas e apropriadas à natureza desses serviços, utilizando princípios,
conhecimentos e técnicas **reconhecidamente fundamentados na ciência
psicológica**, na ética e na legislação profissional;

</Artigo>
<Artigo numero="Art. 1º · d e e" norma="CEPP 2005 · p. 8">

**d)** Prestar serviços profissionais em situações de calamidade pública ou de
emergência, **sem visar benefício pessoal**;

**e)** Estabelecer acordos de prestação de serviços que respeitem os direitos do
usuário ou beneficiário de serviços de Psicologia;

</Artigo>
</Grade>

<!--
A alínea "c" tem duas exigências que a turma lê como uma só: serviço de qualidade
E condições de trabalho dignas. A segunda é sobre o empregador tanto quanto sobre
o profissional, e é a base normativa da recusa de atender em sala sem porta.

"Reconhecidamente fundamentados na ciência psicológica" é a base da exigência de
instrumento validado, e volta no Art. 2º, "f" e "h".

A alínea "d" foi muito citada em 2020–2021 e nas enchentes do Rio Grande do Sul
em 2024: atendimento em emergência é DEVER, e o que a alínea veda é visar
benefício pessoal nessa condição. Não confundir com voluntariado permanente.

A alínea "e" é o contrato, e prepara "f" e "g", na tela seguinte.
-->

---
layout: documento
artigo: Art. 1º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: São deveres fundamentais dos psicólogos
fonte: CFP, 2005, p. 8 — alíneas "f" e "g"
---
**f)** Fornecer, a quem de direito, na prestação de serviços psicológicos,
informações concernentes **ao trabalho a ser realizado e ao seu objetivo
profissional**;

**g)** Informar, a quem de direito, os resultados decorrentes da prestação de
serviços psicológicos, transmitindo **somente o que for necessário** para a
tomada de decisões que afetem o usuário ou beneficiário;

<!--
Duas alíneas que a turma junta e que são coisas diferentes:

"f" é o CONTRATO, no início — o que vou fazer e para quê.
"g" é a DEVOLUTIVA, no fim — e já com o limite escrito, "somente o que for
necessário".

E a expressão que aparece nas duas, e que se repete em "h": A QUEM DE DIREITO. A
turma lê como "a quem pedir". Não é: é quem tem direito àquela informação, que
pode não ser quem a está pedindo. Quem é essa pessoa se decide no Art. 13
(responsáveis) e no Art. 12 (equipe) — e é o que a questão do bloco do sigilo vai
cobrar.
-->

---
layout: default
---
<Grade :cols="2">
<Artigo numero="Art. 1º · h e i" norma="CEPP 2005 · p. 8">

**h)** Orientar a quem de direito sobre os encaminhamentos apropriados, a partir
da prestação de serviços psicológicos, e fornecer, sempre que solicitado, os
documentos pertinentes ao bom termo do trabalho;

**i)** Zelar para que a comercialização, aquisição, doação, empréstimo, guarda e
forma de divulgação do **material privativo do psicólogo** sejam feitas conforme
os princípios deste Código;

</Artigo>
<Artigo numero="Art. 1º · j" norma="CEPP 2005 · p. 9">

**j)** Ter, para com o trabalho dos psicólogos e de outros profissionais,
respeito, consideração e solidariedade, e, quando solicitado, colaborar com
estes, salvo impedimento por motivo relevante;

</Artigo>
</Grade>

<!--
A alínea "h" é a que sustenta a obrigação de entregar declaração de comparecimento
e relatório quando solicitados — "sempre que solicitado". O detalhamento de cada
tipo de documento é a aula 09.

A alínea "i" trata do material privativo: testes, protocolos, manuais. É a face
positiva do Art. 18 (não ceder a leigos) e do Art. 2º, "h". Repare que ela inclui
GUARDA, o que a liga ao Art. 15 e à Res. CFP 001/2009.

A alínea "j" é a da colegialidade, e tem uma válvula: "salvo impedimento por
motivo relevante". Ela não obriga a colaborar sempre — obriga a ter razão para
não colaborar.
-->

---
layout: documento
artigo: Art. 1º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: São deveres fundamentais dos psicólogos
fonte: CFP, 2005, p. 9 — alíneas "k" e "l", as duas últimas
---
**k)** Sugerir serviços de outros psicólogos, sempre que, por motivos
justificáveis, não puderem ser continuados pelo profissional que os assumiu
inicialmente, fornecendo ao seu substituto as informações necessárias à
continuidade do trabalho;

**l)** **Levar ao conhecimento das instâncias competentes** o exercício ilegal ou
irregular da profissão, transgressões a princípios e diretrizes deste Código ou
da legislação profissional.

<!--
A alínea "k" é a do encaminhamento quando se interrompe: não basta parar, é
preciso passar o trabalho adiante, com informação. Ela conversa diretamente com o
Art. 15, § 1º, no bloco do sigilo.

A alínea "l" é a mais desconfortável da aula, e vale nomear isso. Ela transforma o
silêncio diante de uma irregularidade em descumprimento de dever — e o Art. 2º,
"e" fecha o cerco pelo outro lado, vedando a conivência.

A turma vai perguntar se isso obriga a denunciar colega. Obriga a levar às
INSTÂNCIAS COMPETENTES, que é o CRP, não a expor publicamente. A diferença
importa, e é a resposta da segunda questão de múltipla escolha de hoje.
-->

---
layout: default
---
# Art. 1º na prática · a alínea da competência

<Artigo numero="Art. 1º, alínea b" norma="Código de Ética Profissional do Psicólogo, 2005">

Assumir responsabilidades profissionais somente por atividades para as quais
esteja **capacitado pessoal, teórica e tecnicamente**.

</Artigo>

<Nota tipo="alerta" titulo="No serviço">

Uma psicóloga recém-formada é contratada por uma clínica que emite laudos para
porte de arma. Nunca aplicou os instrumentos exigidos e recebe o manual na
véspera. A clínica garante que "é só seguir o passo a passo".

Aceitar transgride a alínea "b" — e, se o laudo sair, também o Art. 2º, "g".

</Nota>

<Fonte>Os instrumentos válidos para essa finalidade estão no SATEPSI. <Norma>Res. CFP 009/2018</Norma></Fonte>

<!--
Caso escolhido de propósito: é a situação de trabalho mais provável para esta
turma nos próximos dois anos, e a pressão vem do empregador, não da própria
ambição.

Diga o que a alínea NÃO diz: ela não exige especialização formal nem título. Exige
capacitação, que se demonstra por formação, supervisão e prática. Um
recém-formado pode atender — sob supervisão, e dentro do que sustenta.

Se a turma perguntar o que fazer: o Art. 1º, "k" e o Art. 6º, "a" dão a saída —
encaminhar a quem seja habilitado. Recusar não é falta; aceitar é.
-->

---
layout: figura
imagem: /deveres-e-vedacoes.svg
legenda: As duas cores da identidade aparecem porque há dois lados a comparar. Nenhuma delas é a cor do "certo".
lado: esquerda
---
# O verbo separa os dois artigos

O Art. 1º e o Art. 2º cobrem quase o mesmo território, e é o **verbo** que os
distingue: um manda fazer, o outro proíbe. Juntos são **29 alíneas** — o miolo do
Código. <Norma>Arts. 1º e 2º · CEPP 2005</Norma>

A consequência é prática: a mesma conduta aparece nos dois. Divulgar material privativo a
leigos descumpre o Art. 1º, "i", e infringe o Art. 18.

<Fonte>CFP, <em>Código de Ética Profissional do Psicólogo</em>, 2005, p. 8–11.</Fonte>

<!--
Trinta segundos de respiro entre dois blocos densos, e um ponto que fica: quando
um caso é analisado, não se procura "o artigo"; procura-se o que ele descumpriu e
o que ele infringiu. As duas listas se sobrepõem de propósito.

Se alguém contar as alíneas e reclamar que são 29 e não 30: sim, 12 + 17. O Art.
1º vai de "a" a "l" (o alfabeto jurídico não usa "ç"), o Art. 2º de "a" a "q".
-->

---
layout: secao
numero: "02"
title: As dezessete vedações
note: Art. 2º, alíneas “a” a “q”. O que o psicólogo não pode fazer.
---

---
layout: default
---
<Grade :cols="2">
<Artigo numero="Art. 2º · a e b" norma="CEPP 2005 · p. 9">

**a)** Praticar ou ser conivente com quaisquer atos que caracterizem
**negligência, discriminação, exploração, violência, crueldade ou opressão**;

**b)** Induzir a convicções políticas, filosóficas, morais, ideológicas,
religiosas, **de orientação sexual** ou a qualquer tipo de preconceito, quando do
exercício de suas funções profissionais;

</Artigo>
<Artigo numero="Art. 2º · c e d" norma="CEPP 2005 · p. 9">

**c)** Utilizar ou favorecer o uso de conhecimento e a utilização de práticas
psicológicas como **instrumentos de castigo, tortura** ou qualquer forma de
violência;

**d)** Acumpliciar-se com pessoas ou organizações que exerçam ou favoreçam o
**exercício ilegal** da profissão de psicólogo ou de qualquer outra atividade
profissional;

</Artigo>
</Grade>

<!--
"Ao psicólogo é vedado" — leia o caput em voz alta, porque ele não está na tela:
está no título do bloco e vale para as dezessete alíneas.

A lista da alínea "a" repete, palavra por palavra, a do Princípio II. Não é
redundância: é a passagem do aspiracional ao exigível, que foi o assunto do
slide 5.

A alínea "b" é a que alcança as chamadas terapias de conversão. Ela sozinha já
bastaria; as Res. CFP 001/1999 e 001/2018 a detalham para orientação sexual e
para pessoas transexuais e travestis. Note que a vedação é INDUZIR A convicção,
em qualquer direção — inclusive a do próprio psicólogo.

A alínea "c" tem lastro histórico: o Código de 1987 foi escrito à sombra da
ditadura, e esta alínea guarda essa memória.

A alínea "d" alcança quem trabalha COM quem exerce ilegalmente — a clínica que
mantém terapeuta sem registro, por exemplo. Acumpliciar-se é menos que praticar.
-->

---
layout: documento
artigo: Art. 2º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: Ao psicólogo é vedado
fonte: CFP, 2005, p. 9–10 — alíneas "e" e "f"
---
**e)** **Ser conivente** com erros, faltas éticas, violação de direitos, crimes
ou contravenções penais praticados por psicólogos na prestação de serviços
profissionais;

**f)** Prestar serviços ou vincular o título de psicólogo a serviços de
atendimento psicológico cujos procedimentos, técnicas e meios **não estejam
regulamentados ou reconhecidos** pela profissão;

<!--
A alínea "e" é o par negativo do Art. 1º, "l": lá o dever de levar adiante, aqui a
vedação de calar. Repare no recorte — a conivência é vedada quanto a atos DE
PSICÓLOGOS, o que faz do Código também um instrumento de controle interno da
categoria. É a resposta da próxima questão de múltipla escolha.

A alínea "f" alcança terapias emergentes e práticas sem reconhecimento. O verbo
importa: VINCULAR O TÍTULO a algo que não é psicologia já é a infração, mesmo que
a atividade em si não seja ilegal para quem não é psicólogo.

Terapias emergentes é um dos quinze temas de seminário do semestre.
-->

---
layout: default
---
<Grade :cols="2">
<Artigo numero="Art. 2º · g, h e i" norma="CEPP 2005 · p. 10">

**g)** Emitir documentos **sem fundamentação e qualidade técnico-científica**;

**h)** Interferir na validade e fidedignidade de instrumentos e técnicas
psicológicas, **adulterar seus resultados** ou fazer declarações falsas;

**i)** **Induzir** qualquer pessoa ou organização a recorrer a seus serviços;

</Artigo>
<Artigo numero="Art. 2º · j" norma="CEPP 2005 · p. 10">

**j)** Estabelecer com a pessoa atendida, familiar ou terceiro, que tenha vínculo
com o atendido, **relação que possa interferir negativamente nos objetivos do
serviço prestado**;

</Artigo>
</Grade>

<!--
"g" e "h" são as duas alíneas do documento e do teste. A fundamentação é o que a
Res. CFP 06/2019 detalha (aula 09); a validade dos instrumentos é o que o SATEPSI
e a Res. CFP 09/2018 organizam (aula 11).

A alínea "i" é curta e vale distinguir de divulgar: anunciar serviços é lícito e
regulado pelo Art. 20; INDUZIR alguém a recorrer a eles, não.

A alínea "j" é a da relação múltipla, e é a que a turma vai encontrar no estágio.
Repare no que ela NÃO diz: não proíbe toda relação fora do enquadre, proíbe a que
POSSA interferir NEGATIVAMENTE NOS OBJETIVOS do serviço. O critério é o serviço,
não a moral do vínculo — e é isso que o slide de exemplo vai trabalhar.
-->

---
layout: documento
artigo: Art. 2º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: Ao psicólogo é vedado
fonte: CFP, 2005, p. 10 — alínea "k"
---
**k)** Ser perito, avaliador ou parecerista em situações nas quais seus **vínculos
pessoais ou profissionais, atuais ou anteriores**, possam afetar a qualidade do
trabalho a ser realizado ou a **fidelidade aos resultados** da avaliação;

<!--
A alínea da perícia, e ela ganha tela própria porque é a mais cobrada em concurso
e a que estrutura a aula 13.

Três coisas:

1. "Perito, avaliador ou parecerista" — três papéis, não um. A vedação não é só
   para a perícia judicial.

2. "ATUAIS OU ANTERIORES". O vínculo encerrado continua contaminando. Não existe
   prazo de carência escrito: o critério é se o vínculo pode afetar a qualidade
   ou a fidelidade.

3. A Res. CFP 008/2010, Art. 10, dá a regra mais conhecida que decorre daqui: é
   vedado ao psicoterapeuta das partes atuar como perito no mesmo caso.

O caso típico do estágio: atender a criança e depois ser chamado a "dar um
parecer" para a audiência de guarda. São dois papéis, e a alínea separa os dois.
-->

---
layout: default
---
<Grade :cols="2">
<Artigo numero="Art. 2º · l e m" norma="CEPP 2005 · p. 10">

**l)** **Desviar** para serviço particular ou de outra instituição, visando
benefício próprio, pessoas ou organizações atendidas por instituição com a qual
mantenha qualquer tipo de vínculo profissional;

**m)** Prestar serviços profissionais a organizações concorrentes de modo que
possam resultar em prejuízo para as partes envolvidas, decorrentes de
**informações privilegiadas**;

</Artigo>
<Artigo numero="Art. 2º · n e o" norma="CEPP 2005 · p. 10">

**n)** **Prolongar, desnecessariamente**, a prestação de serviços profissionais;

**o)** Pleitear ou receber comissões, empréstimos, doações ou vantagens outras de
qualquer espécie, **além dos honorários contratados**, assim como intermediar
transações financeiras;

</Artigo>
</Grade>

<!--
A alínea "l" é a que mais aparece em estágio e em serviço público: levar para o
consultório particular quem se conheceu na UBS ou no serviço-escola. Repare em
"visando benefício próprio" — o encaminhamento honesto, sem captação, é o Art.
1º, "k", e é dever, não infração.

A alínea "m" é da psicologia organizacional e quase nunca é lembrada: consultoria
para duas empresas concorrentes.

A alínea "n" é curta e pesada. Ela põe a duração do trabalho sob escrutínio ético,
o que a turma raramente espera de um código. Não define prazo, e não poderia: o
critério é a necessidade, que se sustenta com registro — Art. 1º, "g", e Res. CFP
001/2009, Art. 2º, III.

A alínea "o" é a do dinheiro por fora, e inclui algo que passa batido:
INTERMEDIAR TRANSAÇÕES FINANCEIRAS. É o que o slide de exemplo vai usar.
-->

---
layout: documento
artigo: Art. 2º
norma: Código de Ética Profissional do Psicólogo · Res. CFP 010/2005
title: Ao psicólogo é vedado
fonte: CFP, 2005, p. 11 — alíneas "p" e "q", as duas últimas
---
**p)** Receber, pagar remuneração ou **porcentagem por encaminhamento** de
serviços;

**q)** Realizar diagnósticos, divulgar procedimentos ou apresentar resultados de
serviços psicológicos **em meios de comunicação**, de forma a expor pessoas,
grupos ou organizações.

<!--
A alínea "p" tem nome no mercado: veda a comissão por indicação, e alcança tanto
quem paga quanto quem recebe. Plataformas que remuneram por encaminhamento estão
exatamente aqui.

A alínea "q" é a de 2005 que envelheceu melhor do que se poderia esperar: escrita
para rádio e televisão, alcança sem esforço o perfil profissional em rede social.

Repare no recorte, porque é ele que decide: ela NÃO proíbe falar de psicologia em
público — isso é o Art. 19, e é positivo. Proíbe fazê-lo DE FORMA A EXPOR
pessoas, grupos ou organizações.

O caso-limite que a turma sempre traz: posso postar um caso se eu mudar os dados?
A pergunta correta não é se está anonimizado, e sim se a pessoa se reconheceria.
Guarde para o Art. 9º, daqui a quinze minutos.
-->

---
layout: default
---
# Art. 2º na prática · a relação que interfere

<Nota tipo="alerta" titulo="No serviço">

Um psicólogo atende um adolescente em consultório. A mãe do rapaz é corretora e
propõe intermediar, sem comissão, a compra da sala onde ele atende. Ele recusa;
ela insiste que é só um favor entre conhecidos.

Aceitar não exige má-fé para infringir: basta que a relação **possa** interferir
nos objetivos do atendimento.

</Nota>

<Fonte>A alínea "o" veda, à parte, intermediar transações financeiras. <Norma>Art. 2º, "o" · CEPP 2005</Norma></Fonte>

<!--
Caso sem vilão, de propósito: a mãe age de boa-fé, e a tentação é tratar a recusa
como grosseria. O ponto é que a alínea diz "POSSA interferir" — é um teste de
risco, não de dano consumado.

Pergunte à turma o que muda se o favor for aceito e o atendimento terminar bem.
Resposta: nada, para efeito do artigo. A vedação é anterior ao resultado.

Segundo movimento, se houver tempo: e se fosse a única corretora da cidade? Aí a
conversa vira a alínea contra a realidade do interior — e a saída honesta é
registrar a situação e levá-la à supervisão, não fingir que o vínculo não existe.
-->

---
layout: default
---
A supervisora vem assinando relatórios de estágios que não acompanhou.

<Grade :cols="2">
<Cartao rotulo="a">

Nada a fazer: responde quem assina.

</Cartao>
<Cartao rotulo="b">

Levar às instâncias competentes; calar é infração.

</Cartao>
<Cartao rotulo="c">

Advertir, e só comunicar se reincidir.

</Cartao>
<Cartao rotulo="d">

Comunicar a coordenação, não o Conselho.

</Cartao>
</Grade>

<Nota v-click tipo="ok" titulo="Resposta: b">

Levar adiante e não ser conivente são a mesma exigência, pelos dois lados. <Norma>Art. 1º, "l" · Art. 2º, "e" · CEPP 2005</Norma>

</Nota>

<!--
Item de aplicação, e o mais importante da primeira metade: obriga a turma a usar
duas alíneas de artigos diferentes sobre o mesmo fato.

- (c) é o distrator forte, e é o que a maioria escolhe. O Código não prevê
  gradação nem advertência prévia entre pares: não há "primeiro converso, depois
  comunico" escrito em lugar nenhum. Conversar é legítimo; condicionar a
  comunicação a uma reincidência é que não tem amparo.
- (a) inverte o Art. 2º, "e" — a conivência é a infração.
- (d) é o distrator de controle e traz um erro de fato: a representação ao CRP
  pode partir de qualquer pessoa, inclusive de outro psicólogo.

Se sobrar tempo: quem assinou o relatório sem acompanhar infringiu o Art. 2º,
"g". A responsabilidade do supervisor é o Art. 17.
-->

---
layout: secao
numero: "03"
title: As resoluções que detalham
note: Três alíneas do Art. 2º têm norma própria, e é nela que está o detalhe que se cobra.
---

---
layout: documento
artigo: Res. CFP 001/2018
norma: Normas de atuação em relação às pessoas transexuais e travestis
title: A alínea “b” tem norma própria
fonte: CFP, Res. CFP nº 1, de 29/1/2018, Arts. 7º e 8º
---

**Art. 7º** As psicólogas e os psicólogos, no exercício profissional, **não
exercerão qualquer ação que favoreça a patologização** das pessoas transexuais e
travestis.

**Art. 8º** É **vedado** propor, realizar ou colaborar, sob uma perspectiva
patologizante, com eventos ou serviços que visem a **terapias de conversão**,
reversão, readequação ou reorientação de identidade de gênero.

<!--
A alínea "b" do Art. 2º veda induzir a convicções "de orientação sexual ou a
qualquer tipo de preconceito". Duas resoluções dizem o que isso significa na
prática, e a redação delas é quase gêmea:

- Res. CFP 001/1999, Art. 3º: os psicólogos "não exercerão qualquer ação que
  favoreça a patologização de comportamentos ou práticas homoeróticas, nem
  adotarão ação coercitiva tendente a orientar homossexuais para tratamentos não
  solicitados". O parágrafo único proíbe colaborar com eventos e serviços que
  proponham "tratamento e cura das homossexualidades".
- Res. CFP 001/2018, Arts. 7º e 8º, na tela — para pessoas transexuais e
  travestis, com o parágrafo único do Art. 7º mandando reconhecer e legitimar a
  autodeterminação em relação à identidade de gênero.

Note o que a resolução de 1999 acrescenta e que a alínea sozinha não diz: o Art.
4º dela veda pronunciamento público que reforce preconceito. Isso já é o Art. 19
do Código, que a turma vê na aula 12.
-->

---
layout: default
---

# As alíneas “g” e “h” · documento e teste

<Grade :cols="2">
<Cartao rotulo="Art. 2º, “g”" titulo="Documento sem fundamentação">

O que conta como fundamentação está na **Res. CFP 06/2019**, que institui as
regras dos documentos escritos e revogou a Res. 007/2003.

</Cartao>
<Cartao rotulo="Art. 2º, “h”" titulo="Interferir na validade do teste">

Que instrumento é válido, e para quê, está no **SATEPSI** — o sistema de
avaliação organizado pela **Res. CFP 009/2018**.

</Cartao>
</Grade>

<Fonte>As duas têm aula própria: documentos na 09, testes na 11. <Norma>Art. 2º, “g” e “h” · CEPP 2005</Norma></Fonte>

<!--
Duas alíneas curtíssimas que a turma lê e acha óbvias, e que na verdade remetem a
dois corpos normativos inteiros.

"Sem fundamentação e qualidade técnico-científica" não é um juízo de gosto: a Res.
CFP 06/2019 diz o que cada tipo de documento deve conter, e é por essa régua que
se afere. Aula 09.

"Interferir na validade e fidedignidade de instrumentos" alcança desde adulterar
resultado até usar um teste fora da finalidade para a qual foi validado. O
SATEPSI é o cadastro que diz quais instrumentos têm parecer favorável. Aula 11.

Repare que o Art. 1º, "c" já pedia técnicas "reconhecidamente fundamentadas na
ciência psicológica". A alínea "h" é a mesma exigência, pelo avesso.
-->

---
layout: default
---

# As alíneas “d” e “k” · quem exerce e quem avalia

<Grade :cols="2">
<Cartao rotulo="Art. 2º, “d”" titulo="Exercício ilegal">

Psicólogo é quem tem registro no CRP. Acumpliciar-se com quem exerce sem registro
é infração, ainda que o serviço não seja seu.

</Cartao>
<Cartao rotulo="Art. 2º, “k”" titulo="Perito com vínculo">

A **Res. CFP 008/2010**, Art. 10, fecha o caso mais comum: é vedado ao
psicoterapeuta das partes atuar como perito no mesmo processo.

</Cartao>
</Grade>

<Fonte>A profissão e os Conselhos são criados pela <Norma>Lei nº 5.766/1971</Norma>. Perícia é a aula 13 do plano.</Fonte>

<!--
A alínea "d" costuma ser lida como se falasse de charlatães distantes. Não é: ela
alcança o psicólogo que trabalha numa clínica que mantém "terapeuta" sem
registro, ou que assina por serviço prestado por quem não pode prestá-lo.

A alínea "k" é a que estrutura toda a discussão de perícia. Três palavras
importam: "atuais OU ANTERIORES". O vínculo encerrado continua contaminando, e não
há prazo de carência escrito — o critério é se o vínculo pode afetar a qualidade
do trabalho ou a fidelidade aos resultados.

O caso típico do estágio: atender a criança e depois ser chamado a "dar um
parecer" para a audiência de guarda. São dois papéis, e a alínea os separa.
-->

---
layout: default
---

Uma psicóloga que atende uma adolescente em terapia é convidada pela família a
escrever um parecer para a ação de guarda em que a mãe disputa a tutela.

<Grade :cols="2">
<Cartao rotulo="a">

Pode: quem melhor conhece o caso é quem atende.

</Cartao>
<Cartao rotulo="b">

Não pode: o vínculo terapêutico afeta a fidelidade da avaliação.

</Cartao>
<Cartao rotulo="c">

Pode, desde que informe o juízo do vínculo.

</Cartao>
<Cartao rotulo="d">

Pode, se a família autorizar por escrito.

</Cartao>
</Grade>

<Nota v-click tipo="ok" titulo="Resposta: b">

O vínculo, atual ou anterior, é o que a alínea veda. <Norma>Art. 2º, “k” · CEPP 2005</Norma>

</Nota>

<!--
Item de aplicação. O erro mais comum é (a), e ele tem uma intuição forte por trás:
quem atende sabe mais. A alínea responde que saber mais não é o problema — é a
impossibilidade de sustentar a fidelidade aos resultados quando há vínculo.

- (c) e (d) partem da ideia de que transparência ou consentimento curam o conflito
  de interesse. Não curam: a vedação é objetiva, e nem a parte nem o juízo podem
  dispensá-la.

O que a psicóloga PODE fazer: emitir, se solicitada e com o devido cuidado, um
documento sobre o atendimento que presta — declaração, relatório — que é coisa
diferente de um parecer avaliativo sobre a disputa. A distinção entre os
documentos é a aula 09; a perícia, a 13.
-->

---
layout: secao
numero: "04"
title: Estudo de caso
note: A situação, e a pergunta que a série repete — houve falta ética, e qual artigo a caracteriza?
---

---
layout: caso
numero: "01"
title: A vaga que apareceu na fila
perguntas:
  - Houve falta ética? Se houve, qual artigo e qual alínea a caracterizam?
  - O que na conduta é decisivo — e o que é apenas desconfortável?
  - O que precisaria mudar no caso para o veredito mudar?
tempo: 12 min
fonte: caso construído para a aula, sem correspondência com situação real
---

Um psicólogo de uma UBS atende, no acolhimento, uma mulher em sofrimento agudo. A
fila para psicoterapia na unidade é de oito meses. Ele diz que atende em
consultório particular, oferece valor social e a encaixa na semana seguinte.

Ela aceita, agradece, e o tratamento corre bem.

<!--
Doze minutos, em grupos de três. Não adiante o veredito: ele está no slide
seguinte, e a turma precisa chegar sozinha à alínea.

O caso é desenhado para ser desconfortável nos dois sentidos: a conduta é
tecnicamente boa, o desfecho é bom, e ainda assim há infração.
-->

---
layout: confronto
kicker: As duas leituras
title: A fila de oito meses
esquerda: Não houve falta
direita: Houve, e é o Art. 2º, “l”
pergunta: O bom resultado do atendimento muda alguma coisa na análise?
---

A mulher estava em sofrimento agudo e a alternativa real era esperar oito meses.
Ele cobrou valor social, atendeu bem e ela melhorou.

Recusar seria escolher a fila em nome de uma formalidade.

::direita::

A alínea veda **desviar para serviço particular**, visando benefício próprio,
pessoa atendida por instituição com a qual se mantém vínculo. Todos os elementos
estão no caso.

O bom desfecho não entra na análise: a vedação é anterior ao resultado.

<!--
Cinco minutos de confronto antes do veredito. A leitura da esquerda não é
ingênua — é a que a turma vai defender, e é a que descreve a realidade da rede.

O que desfaz o impasse não é o mérito do atendimento, é o desenho da vedação: ela
protege o serviço público de virar vitrine do consultório de quem nele trabalha.
Se o desvio fosse lícito quando bem-intencionado, a fila seria um argumento de
venda.

Nomeie o Princípio VII: há assimetria entre quem está em sofrimento agudo diante
de uma fila de oito meses e quem lhe oferece a saída.
-->

---
layout: default
---

# O veredito · e o que teria sido lícito

<Nota tipo="erro" titulo="Houve falta ética">

**Art. 2º, “l”** — desviar para serviço particular, visando benefício próprio,
pessoa atendida por instituição com a qual mantém vínculo profissional.

</Nota>

<Nota tipo="ok" titulo="O que seria lícito">

Encaminhar a outro profissional, sem captação: é o **Art. 1º, “k”**, e é dever.
Levar a fila de oito meses à gestão também: **Art. 1º, “l”**.

</Nota>

<!--
O veredito, e ele precisa vir com a saída — senão a aula ensina paralisia.

Três pontos ao fechar:

1. O elemento "visando benefício próprio" está preenchido mesmo com valor social:
   houve remuneração e houve captação dentro do serviço. Não é preciso lucro alto,
   basta o benefício.

2. Encaminhar para OUTRO profissional é o oposto: o Art. 1º, "k" manda sugerir
   serviços de outros psicólogos quando o trabalho não pode ser continuado. A
   diferença entre a infração e o dever é para quem se encaminha.

3. A fila de oito meses é um problema real, e o Código também alcança isso: o Art.
   1º, "l" manda levar às instâncias competentes o que é irregular, e o Princípio
   III pede a leitura da condição institucional. Registrar a fila e levá-la à
   gestão é conduta prevista, não reclamação.

Se a turma perguntar o que aconteceria num processo ético: a apuração seguiria o
Código de Processamento Disciplinar, e a dosimetria consideraria o valor social e
a ausência de dano. Isso é a aula 13.
-->

---
layout: fecho
kicker: Aula 07
title: O que fica
pontos:
  - 'São <span class="ds-em">29 alíneas</span> — 12 deveres e 17 vedações'
  - 'O <span class="ds-em">verbo</span> separa os dois artigos, não o assunto'
  - 'Três alíneas do Art. 2º têm <span class="ds-em">resolução própria</span>'
---

Leitura: os Arts. 1º e 2º, na íntegra. Na aula 08, os Arts. 3º a 5º — a
instituição, os honorários e a greve.

---
layout: default
---

# Referências

**Normas**

- CFP. **Res. CFP nº 010/05** — o **CEPP**, Arts. 1º e 2º (p. 8–11).
- CFP. **Res. CFP nº 001/1999**, Arts. 3º e 4º · **Res. CFP nº 001/2018**, Arts. 7º e 8º.
- CFP. **Res. CFP nº 06/2019** (documentos escritos) · **Res. CFP nº 009/2018** (testes, SATEPSI).
- CFP. **Res. CFP nº 008/2010**, Art. 10 (perícia e assistência técnica).
- BRASIL. **Lei nº 5.766/1971** — cria o CFP e os CRPs.

<!--
Referências completas:

- CFP. Resolução CFP nº 010/05, de 21 de julho de 2005. Aprova o Código de Ética
  Profissional do Psicólogo. Brasília: CFP, 2005. Edição impressa de agosto/2005,
  20 p. — é a que a aula cita por página (p. 8 a 11).
- CFP. Resolução CFP nº 001, de 22 de março de 1999. Estabelece normas de atuação
  para os psicólogos em relação à questão da orientação sexual. Art. 3º e
  parágrafo único; Art. 4º.
- CFP. Resolução CFP nº 1, de 29 de janeiro de 2018. Estabelece normas de atuação
  para as psicólogas e os psicólogos em relação às pessoas transexuais e
  travestis. Arts. 7º (e parágrafo único) e 8º.
- CFP. Resolução CFP nº 06/2019 — documentos escritos produzidos pela psicóloga ou
  pelo psicólogo; revogou a Res. CFP nº 007/2003. Aula 09.
- CFP. Resolução CFP nº 009/2018 — avaliação psicológica e SATEPSI. Aula 11.
- CFP. Resolução CFP nº 008/2010 — perícia e assistência técnica; o Art. 10 veda
  ao psicoterapeuta das partes atuar como perito no mesmo caso.
- BRASIL. Lei nº 5.766, de 20 de dezembro de 1971. Cria o Conselho Federal e os
  Conselhos Regionais de Psicologia.

Vídeo: nenhum verificado sobre os Arts. 1º e 2º. Termo de busca sugerido, no canal
oficial "Conselho Federal de Psicologia": "Código de Ética deveres e vedações".
Nenhum conteúdo desta aula depende de vídeo.
-->
